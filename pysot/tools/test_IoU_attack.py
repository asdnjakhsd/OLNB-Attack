...
def main():
    ...
    perturb_max = 10000
    total_vot = 0
    mean_vot_path = os.path.join('results', args.dataset, 'SiamRPN++(IoU_attack)', 'baseline', 'mean_vot.txt')
        ...
        for v_idx, video in enumerate(dataset):
            ...
            for idx, (img, gt_bbox) in enumerate(video):
                ...
                if idx == frame_counter:
                    ...
                    print("processing frame: ",idx," in video ", video.name)
                    # black-box IoU attack
                    image = img
                    last_gt = save_bbox
                    # heavy noise image
                    heavy_noise = np.random.randint(
                        -1, 2, (image.shape[0], image.shape[1], image.shape[2])) * 128
                    ...
                    target_score = overlap_ratio(
                        np.array(outputs_orig['bbox']), np.array(outputs_target['bbox']))
                    ...
                    if target_score < 0.8:
                        ...
                        while True:
                            ...
                            outputs_adv = tracker.track_fixed(
                                (trial_sample + 128).astype(np.uint8))
                            # IoU score
                            threshold_1 = overlap_ratio(
                                np.array(outputs_orig['bbox']), np.array(outputs_adv['bbox']))
                            threshold_2 = overlap_ratio(
                                np.array(last_gt), np.array(outputs_adv['bbox']))
                            threshold = para_rate * threshold_1 + \
                                (1 - para_rate) * threshold_2
                        ...
                        while True:
                            ...
                            while True:
                                ...
                                for i in np.arange(10):
                                    trial_sample = adversarial_sample + orthogonal_perturbation(delta,
                                                                                                adversarial_sample,
                                                                                                noise_sample)
                                    trial_sample = np.clip(
                                        trial_sample, -128, 127)
                                    # query
                                    outputs_adv = tracker.track_fixed(
                                        (trial_sample + 128).astype(np.uint8))
                                    # IoU score
                            ...
                            while True:
                                ...
                                trial_sample = adversarial_sample + forward_perturbation(
                                    epsilon *
                                    get_diff(adversarial_sample,
                                             noise_sample), adversarial_sample,
                                    noise_sample)
                                trial_sample = np.clip(trial_sample, -128, 127)
                                # query
                                outputs_adv = tracker.track_fixed(
                                ...
                                threshold_2 = overlap_ratio(
                                    np.array(last_gt), np.array(outputs_adv['bbox']))
                                threshold_sum = para_rate * threshold_1 + \
                                    (1 - para_rate) * threshold_2
                    ...
                    if (idx % 5 == 0):
                        print("print pic")
                        cv2.imwrite("/data/hht/DCT_pysot/pic/pic" + str(idx) + ".jpg", img)       
